...
void main() {
  ...
  group('Watchlist', () {
    test('should get the watchlist status', () async {
      // arrange
      when(mockGetWatchlistStatus.execute(1, "movie"))
    ...
    test('should execute save watchlist when function called', () async {
      // arrange
      when(mockSaveWatchlist.execute(testMovieTable))
          .thenAnswer((_) async => Right('Success'));
      when(mockGetWatchlistStatus.execute(testMovieTable.id, "movie"))
          .thenAnswer((_) async => true);
      // act
      await provider.addWatchlist(testMovieTable);
      // assert
      verify(mockSaveWatchlist.execute(testMovieTable));
    });

    test('should execute remove watchlist when function called', () async {
      // arrange
      when(mockRemoveWatchlist.execute(testMovieTable))
          .thenAnswer((_) async => Right('Removed'));
      when(mockGetWatchlistStatus.execute(testMovieTable.id, "movie"))
          .thenAnswer((_) async => false);
      // act
      await provider.removeFromWatchlist(testMovieTable);
      // assert
      verify(mockRemoveWatchlist.execute(testMovieTable));
    });
    ...
    test('should update watchlist status when add watchlist success', () async {
      ...
      await provider.addWatchlist(testMovieTable);
      // assert
      verify(mockGetWatchlistStatus.execute(testMovieTable.id, "movie"));
      expect(provider.isAddedToWatchlist, true);
